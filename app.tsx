...
// Fix: Import Recharts components directly instead of accessing from window object.
...
// Fix: Add declaration for gapi to resolve TypeScript errors.
...
// Fix: Made the `children` prop optional to resolve multiple TypeScript errors about missing children.
...
// Fix: Extracted props to a named interface to resolve TypeScript error with 'key' prop.
...
// Fix: Explicitly type as React.FC to correctly handle the 'key' prop provided during mapping.
...
const CategoryBreakdownCard = ({ title, categoryTotals, total, currency, type, transactions }: {
    ...
    // The entire content block is memoized to prevent re-rendering when the parent (DashboardTab) re-renders due to date changes.
    // The animation container inside CategoryBreakdownRow will handle its own animations smoothly.
    const content = useMemo(() => (
...
const LoanCard: React.FC<LoanCardProps> = ({ loan, currency, onEdit }) => {
    // FIX: Removed useMemo to ensure paidAmount is always recalculated on re-render, fixing progress bar update issue.
    const paidAmount = loan.payments.reduce((sum, p) => sum + p.amount, 0);
        ...
        <div className="space-y-6">
            ...
            {activePlannerTab === 'planner' && (
                <div className="space-y-6 animate-fade-in">
                    {/* Fix: Corrected prop name from `setPlannerDateFilter` to `setDateFilter` to match ProjectionSummaryProps. */}
